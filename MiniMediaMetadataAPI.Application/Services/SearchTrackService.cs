...
public class SearchTrackService
{
        ...
        ProviderType provider)
    {
        ...
        if (provider is ProviderType.Any or ProviderType.Spotify)
        {
            var spotifyArtists = await _spotifyRepository.SearchTrackByTrackIdAsync(trackId);
            searchResult.AddRange(spotifyArtists?.Select(track => new SearchTrackEntity
            {
                ProviderType = ProviderType.Spotify,
                Id = track.TrackId,
        ...
        if (provider is ProviderType.Any or ProviderType.Tidal && int.TryParse(trackId, out int intTrackId))
        {
            var tidalTracks = await _tidalRepository.SearchTrackByTrackIdAsync(intTrackId);
            searchResult.AddRange(tidalTracks?.Select(track => new SearchTrackEntity
            {
                ProviderType = ProviderType.Tidal,
                Id = track.TrackId.ToString(),
        ...
        if (provider is ProviderType.Any or ProviderType.MusicBrainz && Guid.TryParse(trackId, out Guid guidTrackId))
        {
            var musicBrainzArtists = await _musicBrainzRepository.SearchTrackByTrackIdAsync(guidTrackId);
        ...
        if (provider is ProviderType.Any or ProviderType.Deezer && long.TryParse(trackId, out long longTrackId))
        {
            var deezerTracks = await _deezerRepository.SearchTrackByTrackIdAsync(longTrackId);
            searchResult.AddRange(deezerTracks?.Select(track => new SearchTrackEntity
            {
                ProviderType = ProviderType.Deezer,
                Id = track.TrackId.ToString(),
        ...
        {
            //not suported, keeping it here in case Discogs applies Id's to their tracks or I fix it somehow by generating Id's
        }
